...
CONFS = 3  # TODO move to config
# TODO separate logger from flask one


def getNewAddress():
    client = AuthServiceProxy(CFG.get('plugins', 'BITCOIN'))
    return client.getnewaddress()


def validateAddress(address):
    return address[0:1] in '123m'
...
def processReceive(txid, details, confirmed=False):
    ...
    credit = models.Credit(amount=amount, address=details['address'], currency='BTC', network="Bitcoin", state=state, reference='tx received', ref_id=txid, user_id=addy.user_id)
    ses.add(credit)
    bal = ses.query(models.Balance).filter(models.Balance.user_id == addy.user_id).filter(models.Balance.currency == 'BTC').order_by(models.Balance.time.desc()).first()
    if state == 'complete':
    ...
    if typ == 'transaction' and args.data is not None:
        ...
        for p, put in enumerate(txd['details']):
            if put['category'] == 'send':
                ...
                processReceive("%s:%s" % (txid, p), put, txd['confirmations'] >= CONFS)
